...
export async function loadSales(){
  ...
  $('#salesBody').innerHTML = rows.map(s=>{
    const p = products.find(x=>x.sku===s.sku); const profit = s.revenue - (s.costAtSale*s.qty);
    return `<tr><td>${s.date}</td><td>${s.sku} — ${p?p.name:''}</td><td class='right'>${s.qty}</td><td class='right'>${PKR(s.unitPrice)}</td><td class='right'>${PKR(s.lineDiscount||0)}</td><td class='right'>${PKR(s.revenue)}</td><td class='right'>${PKR(s.costAtSale)}</td><td class='right'>${PKR(profit)}</td></tr>`;
  }).join('');
}

// Quick single-line sale
